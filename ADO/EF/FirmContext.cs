 ...
 *  Состоит из коллекции DbSet ( аналог List. но с доп. возможностями),
 * каждая из которых отвечает за табицу БД
 *  Классы - сущноси должны быт созданы. БД пока не существует (создаем новую
 * строку подключения в App. указываем не существующий файл)
 *  
 * Построение контекста (при выполнении программы) происходит в несколько этапов
 ...
 * - OnModelCreating - создание моделей. Здесь мы задаем начальные значение.
 *   которые попадут в БД при ее создании.
 * 
 * !! База данных на данном этапе отсутсвует. существует строка подключения,
 * но к несуществующей БД
 * Дальнейшая работа с EF делится на два пожхода:
 *  1. использование миграций
 *  2. управление из кода
 *  Подходы не совместимы. нужно следовать одному из них
 *  
 *  Миграции - "снимки", "точки восстановления" - набор инструкций,
 *  создающий или отменяющий найденные в коде (контексте) изменения
 *  Миграции создаются командой в консоли менеджера пакетов (PM)
 *  PM> Add-Migration initial ИмяМиграции
